...
static const char *TAG = "WIFI-D";
static esp_netif_t* _esp_netif_sta = NULL;
static esp_netif_t* _esp_netif_ap = NULL;
//AP名称和密码
static char AP_WIFI_SSID[32] = "ESP-Drone"; //AP模式下创建的WiFi名称
static char AP_WIFI_PWD[64] = "kexie555good" ;  //AP模式下创建的WiFi密码

//STA名称
#define STA_WIFI_SSID "su" //AP模式下创建的WiFi名称
#define STA_WIFI_PASS "12345678kexie-good"   //AP模式下创建的WiFi密码
...
void wifi_init_ap(void) //AP模式初始化
{
	...
	//wifi_config.ap.channel  = 13;//AP通道   (注释之后发现比原来更容易连接到wifi)
	...
	printf("设置无线操作模式为AP ");
	if(esp_wifi_set_mode(WIFI_MODE_APSTA) == ESP_OK) printf("成功\n");//设置无线操作模式为AP和STA
	else printf("失败\n");
